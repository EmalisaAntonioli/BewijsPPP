...
```{r}
itemFrequencyPlot(x = transactions, topN = 10, type = "absolute")
```
...
selection <- which (colnames(table) %in% c("PARTY BUNTING", "WHITE HANGING HEART T-LIGHT HOLDER"))
...
```{r}
transaction_rules <- apriori(data = transactions, parameter = list(support = 0.001, confidence =0.9))
```
