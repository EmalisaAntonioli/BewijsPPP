...
```{r dygraph}
dy_window <- c("2020-01-02", "2020-02-01")

dygraph(data = metals, 
        xlab = "Time",
        ylab = "Gold, Silver, Platinum, and Palladium Performance") %>%
  dyRebase(value=100) %>% # normalize the values relative to 100 
  dyRangeSelector(dateWindow = dy_window) # have the default view zoom in on a time period
```
...
```{r}
# because these two graphs use the same var for the window, they both move when you slide the window
