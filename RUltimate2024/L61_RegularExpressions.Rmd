...
```{r}
str_locate(string = string, pattern = pattern) # only finds one instance
...
Search for words starting with "R".

```{r}
pattern <- "^R" # starting with R
str_detect(string = string, pattern = pattern)
```

Search for words with an "y" at the second position.
...
pattern <- "\\d{3}" # we are looking for 3 consecutive digits
