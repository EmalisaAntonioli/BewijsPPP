...
happiness_mod %>%
  ...
  plot_ly(x= ~economy_gdp,
          y= ~health_life_exp,
          size= ~Freedom^5, # adjusted to make the differences more visible
          color = ~happiness_score,
          text= ~Country,
          hoverinfo='text') %>%
  add_markers() %>%
  colorbar(title='Happiness Score') %>%
  layout(title='Happiness Score 2015',
         xaxis=list(title='Economy (repr by GDP)'),
         yaxis=list(title='Health (repr. by Life Expectancy)'))
```
...
```{r}
subplot(titleX = TRUE, shareY = T,
        happiness_mod %>%
  filter(year == 2015) %>% # this filter is the only difference between the subplots
  plot_ly(x= ~economy_gdp,
          y= ~health_life_exp,
          size= ~Freedom^5, 
          color = ~happiness_score,
  ...
  add_markers() %>%
  colorbar(title='Happiness Score') %>%
  layout(title='Happiness Score 2015/2016',
         xaxis=list(title='Economy (repr by GDP)'),
         yaxis=list(title='Health (repr. by Life Expectancy)')))
```
