...
All my result are a factor 100 more than the solutions, i am not sure why.
...
summary(na.omit(cars_filt))
```

## Train / Validation / Test Split

```{r}
train <- na.omit(cars_filt) # i had nas, so i quickly omitted those
```
...
model_lasso <- glmnet::cv.glmnet(x = train_x, y = train_y, family = "gaussian", 
                                 alpha = 1, lambda = lambdas, nfolds = 10)
```
...
```{r}
lasso_coef <- predict(model_lasso, s= lambda_opt, newxv = val_x, type = "coefficients")
lasso_coef 
```
...
```{r}
model_ridge <- cv.glmnet(x = train_x, y=train_y, family = "gaussian",
