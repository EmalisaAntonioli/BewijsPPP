...
```{r}
VIM::aggr(x = credit_approval, sortVars=T)
```
...
```{r}
credit_approval_rem_NAs <- credit_approval %>%
  na.omit()
```
...
credit_approval_impute$Age <- Hmisc::impute(x=credit_approval_impute$Age) # default median is used
```
...
credit_all_models <- rbind(
  credit_approval_rem_NAs %>% add_column(method = "Removed NA"),
