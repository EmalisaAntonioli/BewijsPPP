...
```{r}
library(readxl)
affairs_xlsx <- read_xlsx("./data/Affairs.xlsx")
```
...
```{r}
library(jsonlite)
affairs_json <- read_json("./data/Affairs.json", simplifyVector = T)
```
...
```{r}
library(foreign)
affairs_spss <- read.table("./data/Affairs.sps", sep = ",")
```
...
```{r}
# using alternative package because tabulizer didn't work
