...
corrplot(cor_airfoil)
```

Which two variables correlate most to target variable?

```{r}
# this shows the two most correlating vars
cor_airfoil[, 6] %>% sort %>% head(2)
```
...
```{r}
# create predictions
airfoil_noise$spl_pred <- predict(object = model,
                                  newdata = airfoil_noise)
...
```{r}
model_freq_displ_chord <- lm(formula = spl ~ frequency + ss_displacement_thickness + chord_length,
                            data = airfoil_noise)
```
...
```{r}
# create predictions
airfoil_noise$spl_pred2 <- predict(object = model_freq_displ_chord,
                                   newdata = airfoil_noise)

# perform model performance
postResample(pred = airfoil_noise$spl_pred2,
             obs = airfoil_noise$spl)
```
