...
outlier_limit <- iris %>%
  ...
  summarise(Q1 = quantile(Petal.Length, probs = 0.25),
            ...
            IQR = Q3 - Q1,
            upper_inner_limit = Q3 + 1.5 * IQR, # 1.5 is the normal def, but can be changed
            lower_inner_limit = Q1 - 1.5 * IQR) 
tbl_df(outlier_limit)
```
...
```{r}
iris <- left_join(iris, outlier_limit, by = 'Species') %>%
  select(-Q1, -Q3, -IQR)
```
...
```{r}
iris <- iris %>%
  mutate(PetalLengthOutlier = ifelse(Petal.Length > lower_inner_limit & Petal.Length < upper_inner_limit, 
                                     "no outlier", "outlier"))
