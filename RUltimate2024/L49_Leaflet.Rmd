...
```{r}
lf <- leaflet() %>%
  addProviderTiles("Stamen.Toner") %>% # Google different types of visualizations
  setView(lng = median_location$long, lat = median_location$lat, zoom=17) %>%
  addCircles(lng = deaths$long, lat = deaths$lat, radius = deaths$count*2, # multiplied by 2 to emphasize differences
             stroke = F,
             color = "red",
             fillOpacity = 0.8,
